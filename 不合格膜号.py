...
for j in col_of_index:
    for i in range(row):
        ...
        if type(cur) == float:
            for y in range(1, 5):  # 可能写错了！
                for x in range(0, 4):  # 可能也写错了！
                    rValue = sheet.cell_value(i + x, j + y)
...
for i in range(index + 1):
    if d[i][1][1] >= 0.15:  # 筛选CV过大
        er += 2
        if er >= el:
            sheet1.write(n, 0, d[i][0])
            i += 1
            n += 1
        for j in range(4, 8):  # 筛选T位置偏移
            if downT <= d[i][1][j] <= upT:
                er += 1
                if er >= el:
                    sheet1.write(n, 0, d[i][0])
                    i += 1
